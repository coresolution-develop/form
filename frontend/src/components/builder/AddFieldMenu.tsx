...
export function AddFieldMenu({ formId, onCreated }: Props) {
  ...
  return (
    <div className="flex items-end gap-2 rounded-lg border border-line bg-white p-3">
      <label className="flex flex-1 flex-col gap-1 text-sm">
        <span className="font-medium text-ink-700">필드 타입</span>
        <select
          value={type}
          onChange={(e) => setType(e.target.value as FieldType)}
          className="h-10 rounded-lg border border-line-input px-2 text-sm focus:outline-none focus:border-brand focus:shadow-focus"
        >
          {TYPES.map((t) => (
