  ...
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-white">
      <div className={headerImageUrl ? 'p-6' : 'p-6 pb-0'}>
        ...
        <h2 className="break-words text-xl font-semibold text-ink-900">{title}</h2>
        {description && (
          <p className="mt-1 whitespace-pre-wrap break-words text-sm text-ink-500">{description}</p>
        )}
      </div>
      {headerImageUrl && (
        // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="p-6">
        <div className="flex flex-col gap-5">
          {fields.length === 0 ? (
            <p className="text-sm text-ink-300">필드가 없습니다.</p>
          ) : (
