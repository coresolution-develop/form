...
export default async function ThanksPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 bg-surface-page px-4 text-center">
      <div className="overflow-hidden rounded-xl border border-line bg-white">
        <div className="h-[5px] bg-brand" />
        <div className="px-8 py-12">
          <h1 className="text-xl font-semibold tracking-[-0.45px] text-ink-900">응답해 주셔서 감사합니다 🎉</h1>
          {title && <p className="mt-2 text-sm text-ink-500">‘{title}’에 정상적으로 제출되었습니다.</p>}
          <p className="mt-1 text-xs text-ink-300">응답은 한 번만 제출할 수 있습니다.</p>
