...
export default function AdminUserDetailPage() {
  ...
  if (isError || !data) {
    return (
      <div className="py-20 text-center">
        <p className="text-ink-400">사용자를 불러올 수 없습니다.</p>
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        <Link href="/admin/users" className="text-sm text-ink-400 hover:text-ink-700">
          ← 사용자 목록
        </Link>
        <div className="mt-1 flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-ink-900">{data.nickname}</h1>
            <p className="text-sm text-ink-400">{data.email}</p>
          </div>
      ...
      <section className="rounded-xl border border-line bg-white p-5">
        ...
        {data.status === 'SUSPENDED' && data.suspendedReason && (
          <p className="mt-4 rounded-lg bg-danger-bg px-4 py-2 text-sm text-danger-fg">
      ...
      {/* 보유 폼 */}
      <section>
        <h2 className="mb-3 text-lg font-semibold text-ink-900">보유 폼 ({data.formCount})</h2>
        {data.forms.length === 0 ? (
            ...
            <table className="min-w-full divide-y divide-line-softer text-sm">
              <thead className="bg-surface-subtle">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right font-medium text-ink-500">공개 폼</th>
                </tr>
              </thead>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <dd className="mt-0.5 text-ink-700">{children}</dd>
    </div>
  );
}
