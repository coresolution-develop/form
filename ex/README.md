...
## 시트 계약
**`근무표-YYYY-MM` 탭 (월별 그리드, 양방향)** — 달마다 한 탭, 과거 탭은 아카이브로 보존
```
탭 이름       = 근무표-YYYY-MM (예: 근무표-2026-07)
A1            = 그 탭의 월 "YYYY-MM"
2행(헤더)     = A:empId | B:성명 | C:직급 | D..:날짜(1..N) | 뒤:합계열(M HD / Y)
3행~          = 직원 1명 = 1행
```
- 탭 이름은 `SHEET_TAB`(접두사, 기본 `근무표`) + `-월`. 동기화·reconcile·onEdit 모두 월→탭으로 타겟.
- **다음 달 준비**: `POST /schedule/months/roll` → 다음 달 탭 생성 + 명부 채움(코드 빈칸) + `activeMonth` 전환. (UI `＋ 다음 달 준비` 버튼)
**`설정` 탭 (근무형태 세팅, 양방향)**
```
...
| POST | `/schedule/reconcile?month=` | 월별 탭 재동기화(그 달 배정만 정리, 직원은 보존) |
| POST | `/schedule/months/roll` | 다음 달 탭 생성 + 명부 채움 + activeMonth 전환 |
