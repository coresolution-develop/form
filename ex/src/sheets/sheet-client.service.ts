...
/**
 * Google Sheets 저수준 클라이언트. 인증(서비스계정)은 전역 1개,
 * 대상 스프레드시트(spreadsheetId)는 조직별로 메서드 인자로 받는다(멀티테넌트).
 */
@Injectable()
export class SheetClientService implements OnModuleInit {
  ...
  async ensureTab(spreadsheetId: string, title: string): Promise<boolean> {
  ...
  async tabExists(spreadsheetId: string, title: string): Promise<boolean> {
    const meta = await this.sheets.spreadsheets.get({
      spreadsheetId,
      fields: 'sheets.properties.title',
    });
  ...
  /** 탭 A1 의 월("YYYY-MM") 읽기 */
  async readMonth(spreadsheetId: string, tab: string): Promise<string | null> {
  ...
  async appendRow(spreadsheetId: string, tab: string, values: any[]) {
  ...
  async readSettings(
    ...
    const res = await this.sheets.spreadsheets.values.get({
      spreadsheetId,
      range: `${settingsTab}!A${SET_DATA_START_ROW}:${lastCol}`,
    });
    const rows = res.data.values ?? [];
    return rows
  ...
  async writeSettings(
    ...
    await this.sheets.spreadsheets.values.clear({
      spreadsheetId,
      range: `${settingsTab}!A${values.length + 1}:${lastCol}1000`,
    });
  }
}
